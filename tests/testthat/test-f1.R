...
test_that("f1_fails", {
  ...
  expect_error(
    f1(data = as.matrix(dip2[dip2$batch %in% c("b0", "b4"), 5:8]),
       tcol = 5:8, grouping = "batch", useEMA = "no",
  ...
  expect_error(
    f1(data = dip2[dip2$batch %in% c("b0", "b4"), ],
       tcol = 5:6, grouping = "batch", useEMA = "no",
       lorellim = 1, uprellim = 85),
    "tcol must be an integer vector")
  expect_error(
    f1(data = dip2[dip2$batch %in% c("b0", "b4"), ],
       tcol = 5:8 + 0.1, grouping = "batch", useEMA = "no",
       lorellim = 1, uprellim = 85),
    "tcol must be an integer vector")
  expect_error(
    f1(data = dip2[dip2$batch %in% c("b0", "b4"), ],
  ...
  expect_error(
    f1(data = dip2[dip2$batch %in% c("b0", "b4"), ],
       tcol = 5:8, grouping = 5, useEMA = "no",
       lorellim = 1, uprellim = 85),
    "grouping must be string")
  expect_error(
    f1(data = dip2[dip2$batch %in% c("b0", "b4"), ],
       tcol = 5:8, grouping = "lot", useEMA = "no",
       lorellim = 1, uprellim = 85),
  ...
  expect_error(
    f1(data = dip2[dip2$batch %in% c("b0", "b4"), ],
       tcol = 5:8, grouping = "batch", useEMA = "maybe",
       lorellim = 1, uprellim = 85),
    "specify useEMA either as \"yes\" or \"no\"")
  expect_error(
    f1(data = dip2[dip2$batch %in% c("b0", "b4"), ],
       tcol = 5:8, grouping = "batch", useEMA = 0,
       lorellim = 1, uprellim = 85),
  ...
  expect_error(
    f1(data = tmp2,
       tcol = 5:8, grouping = "batch", useEMA = "yes",
       lorellim = 1, uprellim = 85),
...
test_that("f1_warns", {
  ...
  expect_error(
    expect_warning(
      f1(data = dip2[dip2$batch %in% c("b0", "b1"), ],
         tcol = 6:8, grouping = "batch", useEMA = "no",
         lorellim = 1, uprellim = 85),
